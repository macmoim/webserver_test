...
function rest_get_image_list_new($category) {
	$image_info = array ();
	...
	$sql_query = "SELECT posts.id as p_id, posts.user_id as p_user_id, title, upload_date, thumb_img_path, rank, profiles.user_name as profile_user_name
						, count(like_bool) as likes_sum
	                   FROM posts 
                       LEFT JOIN profiles ON posts.user_id = profiles.user_id
                       LEFT JOIN likes ON likes.post_id = posts.id AND likes.like_bool = '1'";
...
function rest_get_image_list_by_timestamp_new($category, $timestamp) {
	$image_info = array ();
